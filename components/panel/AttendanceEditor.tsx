...
export default function AttendanceEditor({
  ...
  const save = async () => {
    if (!canMark) {
      setError('این جلسه قابل ثبت نیست.');
      return;
    }
    setSaving(true);
    setError('');
    ...
    try {
      ...
      setPayload(saved);
      setOk('ذخیره شد. برای غیبت و تأخیر جدید پیامک ارسال می‌شود.');
    } catch (e) {
  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-bold text-slate-400">
            <Link href={backHref} className="hover:text-[#7c3aed]">
              بازگشت به کلاس
            </Link>
          </p>
      ...
      <div className="sticky bottom-3 flex gap-2 rounded-2xl border-2 border-slate-200 border-b-4 bg-white p-3 dark:border-slate-800 dark:bg-[#131f24]">
        ...
        <Button className="flex-1" onClick={save} disabled={saving || rows.length === 0 || !canMark}>
